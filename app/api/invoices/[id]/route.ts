...
export async function PATCH(
  ...
  try {
    ...
    const isProofUpdate = 'proof_url' in validatedData;

    // Only allow content updates on pending/draft invoices (except proof_url which can be updated anytime)
    if (!isStatusUpdate && !isProofUpdate && !['pending', 'draft'].includes(existing.status)) {
